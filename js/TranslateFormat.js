function ATOMfactorizeMinus($startNode){//translate from (-(a)) to (-1)(a)
	...
	attachEventsAndExtend($cloneMinus);
	$clone[0].ATOM_setName("1");
	$cloneMinus.insertAfter($startNode);
	$cloneMinus[0].ATOM_getRoles().append($clone);
	$startNode[0].ATOM_dissolveContainer()   //remove minus from $startNode  
	refreshInfix($extOp);
	refreshGlued();
	ssnapshot.take();
}
...
function signsAsClassesSubtree($startNode,mode){
	...
	$startNode.find('[data-atom]').each(function(index) {// tutti gli HTML nodes con classe .ATOM
		signsAsClasses($(this),mode);
	})
}


function signsAsClasses($atom,mode/* SignsInNames_to_SignsAsClasses SignsAsClasses_to_SignsInNames SignsAsClasses_to_MinusOp MinusOp_to_SignsAsClasses*/){// <>-a<> to <class="minus">a<>
	...
	if( mode == "SignsInNames_to_SignsAsClasses" ){	
		if(name[0] === "/"){
			name = name.substr(1) //nome privato del segno meno
		...
		if( $atom.attr('data-atom') === "minus" && $ATOMchildren.length == 1  ){// i minus che hanno un solo children
	...
	$atom.attr("data-atom", (isNaN(name))?"ci":"cn" )// se numero allora classe "cn"
}

//**** glued
var glueFunctions=["minus","m_inverse","not"]
function refreshGlued($startNode){//marca con classe glued gli atomi contenuti in un "minus" ecc..
	//get starting point
