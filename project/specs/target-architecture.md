...
flowchart TB
    ...
    ENODES["ENODEs API<br/>manipolazione albero + refresh visivo<br/>(unico punto di accesso al DOM-modello)"]
...
Flusso tipico di un'azione: l'utente agisce sulla UI → il GESTURE MANAGER riconosce il gesto e produce un intento → MAIN lo instrada al motore (dispatch per nome via registro) → il motore calcola la trasformazione e la applica tramite ENODEs API → MAIN conclude (PActx): chiede lo snapshot a UNDO e l'esito alla SESSIONE DIDATTICA.
